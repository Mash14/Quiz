...
function Comics() {
    ...
    useEffect(()=>{
        if(questionType === 'easy') {
            ...
            fetch('https://opentdb.com/api.php?amount=10&category=29&difficulty=medium&type=multiple')
                .then(response => response.json())
                .then(res => setQuestions(res.results))
        } else if (questionType === 'hard') {
            fetch('https://opentdb.com/api.php?amount=10&category=29&difficulty=hard&type=multiple')
                .then(response => response.json())
                .then(res => setQuestions(res.results))
        }      
